...
- Nube opcional (Firebase): `firebase-config.js` (null = desactivada) + `cloud.js` (módulo ES, expone `window.Cloud`). Con la nube activa: login con Google obligatorio para corregir (`requireLogin()`), cada corrección se guarda en la colección Firestore `corrections` con los errores parseados de la tabla 🔍 (`parseErrorsFromMarkdown()`), y la tarjeta "Corpus de errores frecuentes" agrega los errores de todos por tipo/nivel. Las reglas de Firestore están en el README. Si el usuario pasa el objeto `firebaseConfig`, pegarlo en firebase-config.js.
